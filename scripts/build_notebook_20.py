...
def run():
    ...
    print(
        f"p-value: {top_pair.p_value:.5f} | ADF stat: {top_pair.test_stat:.4f} | Half-life: {top_pair.half_life:.1f} days"
    )
    ...
    signals = generate_pairs_signals(
        zscore, entry_threshold=2.0, exit_threshold=0.2, stop_loss_threshold=3.5
    )

    # Plot 1: Pairs Spread, Rolling Beta, and Z-Score Signals
    fig, axes = plt.subplots(
        3, 1, figsize=(14, 11), sharex=True, gridspec_kw={"height_ratios": [1.2, 1, 1.4]}
    )
    ...
    axes[0].set_title(
        f"Cointegrated Pair: {asset_y} vs {asset_x} — Normalized Price Trajectories",
        fontsize=13,
        fontweight="bold",
    )
    ...
    axes[1].plot(
        rolling_hr.index,
        rolling_hr["hedge_ratio"],
        label=r"60-Day Dynamic Rolling $\beta_t$",
        color="#2ca02c",
        lw=1.8,
    )
    axes[1].axhline(
        ...
        linestyle="--",
        lw=1.5,
        label=f"Static Full-Sample $\\beta = {static_beta:.3f}$",
    )
    axes[1].set_title(
        r"Dynamic Hedge Ratio ($\beta_t$) Over Time — Eliminating Look-Ahead Bias",
        fontsize=12,
        fontweight="bold",
    )
    ...
    axes[2].axhline(
        2.0, color="#d62728", linestyle="--", lw=1.2, label=r"Short Entry Threshold ($+2\sigma$)"
    )
    axes[2].axhline(
        -2.0, color="#2ca02c", linestyle="--", lw=1.2, label=r"Long Entry Threshold ($-2\sigma$)"
    )
    ...
    axes[2].axhline(
        -3.5, color="#8c564b", linestyle="-.", lw=1.2, label=r"Stop Loss ($-3.5\sigma$)"
    )
    ...
    if len(long_pos) > 0:
        axes[2].scatter(
            ...
            s=25,
            alpha=0.7,
            label="Long Spread Active",
        )
    if len(short_pos) > 0:
        axes[2].scatter(
            short_pos,
            zscore.loc[short_pos],
            color="#d62728",
            marker="v",
            s=25,
            alpha=0.7,
            label="Short Spread Active",
        )

    axes[2].set_title(
        r"Spread Z-Score & State-Machine Trading Bands (OU Half-Life: "
        + f"{top_pair.half_life:.1f} days)",
        fontsize=12,
        fontweight="bold",
    )
    ...
    ax_eq.plot(
        bt_result.portfolio_equity.index,
        bt_result.portfolio_equity,
        label="Market-Neutral Pairs Strategy (Net of 5bps costs)",
        color="#1f77b4",
        lw=2,
    )
    ...
    ax_eq.plot(
        bench_eq.index,
        bench_eq,
        label=f"50/50 Buy & Hold Benchmark ({asset_y}/{asset_x})",
        color="#7f7f7f",
        linestyle="--",
        lw=1.5,
    )
    ax_eq.set_title(
        f"Pairs Trading Performance vs Benchmark ({asset_y} / {asset_x})",
        fontsize=13,
        fontweight="bold",
    )
    ...
    # Underwater drawdown
    dd_strat = (
        bt_result.portfolio_equity - bt_result.portfolio_equity.cummax()
    ) / bt_result.portfolio_equity.cummax()
    ax_dd.fill_between(
        dd_strat.index,
        dd_strat * 100,
        0,
    ...
    risk_engine.evaluate_order(
        OrderIntent(
            "MSFT", OrderAction.BUY, quantity=200, price=100.0, timestamp="2024-06-03 09:30:00"
        ),
        p_state1,
    )
    ...
    risk_engine.evaluate_order(
        OrderIntent(
            "NVDA", OrderAction.BUY, quantity=300, price=100.0, timestamp="2024-06-03 10:00:00"
        ),
        p_state1,
    )
    ...
    risk_engine.evaluate_order(
        OrderIntent(
            "TSLA", OrderAction.BUY, quantity=100, price=100.0, timestamp="2024-06-03 11:00:00"
        ),
        p_state1,
    ...
    risk_engine.evaluate_order(
        OrderIntent(
            "GOOGL", OrderAction.BUY, quantity=100, price=100.0, timestamp="2024-06-04 10:15:00"
        ),
        p_state_dd,
    )
    ...
    risk_engine.evaluate_order(
        OrderIntent(
            "AAPL", OrderAction.SELL, quantity=100, price=100.0, timestamp="2024-06-04 10:30:00"
        ),
        p_state_dd,
    )
    ...
    risk_engine.evaluate_order(
        OrderIntent(
            "SPY", OrderAction.BUY, quantity=50, price=100.0, timestamp="2024-06-05 13:00:00"
        ),
        p_state_daily,
    )
    ...
    axes[0].axhline(
        ...
        linestyle="--",
        lw=1.8,
        label="15% Max Drawdown Kill Level ($85,000)",
    )
    axes[0].set_title(
        "Risk Engine Defense Gating Under Simulated Stress Regimes", fontsize=13, fontweight="bold"
    )
    ...
    table_data = [
        [
            ...
            r["status"],
            r["rule_triggered"],
            f"${r['requested_value']:,.0f}",
            f"${r['approved_value']:,.0f}",
        ]
    ...
    table.scale(1.0, 1.4)
    axes[1].set_title(
        "Risk Engine Audit Trail: Interventions, Downscalings & Halts",
        fontsize=11,
        fontweight="bold",
        pad=15,
    )
                ...
                "outputs": [
                    {
                        "name": "stdout",
                        "output_type": "stream",
                        "text": ["Modules and environment initialized successfully.\n"],
                    }
                ],
                "source": [
                    "import os\n",
